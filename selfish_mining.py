...
class SelfishMining():
    def selfish_mine(alpha, gamma, N_events):
            ...
            return float(NumberOfSelfishMineBlock)/LongestChainLength, NumberOfSelfishMineBlock, LongestChainLength-NumberOfSelfishMineBlock

def main():
    #varies these params to see different results
    alpha=0.35 #hashing power selfish miner hv, 1-alpha will be the rest
    gamma=0.5 #probability selfish miner wins a blk
    N=10**3 #no of mining events

    print("Theoretical probability :",(alpha*(1-alpha)**2*(4*alpha+gamma*(1-2*alpha))-alpha**3)/(1-alpha*(1+(2-alpha)*alpha)))
    print("Simulated probability, blocks length by selfish miners and honest miners respectively:\n", SelfishMining.selfish_mine(alpha,gamma,N))
